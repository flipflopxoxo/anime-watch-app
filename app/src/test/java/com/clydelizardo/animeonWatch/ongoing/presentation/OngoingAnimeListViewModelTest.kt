...
import com.clydelizardo.animeonWatch.presentation.ongoing.OngoingAnimeViewState
...
class OngoingAnimeListViewModelTest {
    ...
    fun `given view model is created and use case returns success, then state is populated`() =
        runTest {
            ...
            Assert.assertEquals(OngoingAnimeViewState(isLoading = false, animeList = value), viewModel.state.first())
        }
